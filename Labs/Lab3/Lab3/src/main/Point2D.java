...
public class Point2D {
  ...
  /**
   * Compute and return the Euclidean distance between this point
   * and another point specified by its x and y coordinates.
   *
   * @param x1 the x-coordinate of the other point
   * @param y1 the y-coordinate of the other point
   * @return the Euclidean distance between this point and (x1, y1)
   */

  public double getDistance(double x1, double y1) {
    return Math.sqrt(Math.pow(this.getX() - x1, 2)
        + Math.pow(this.getY() - y1, 2));
  }
